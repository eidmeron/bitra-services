...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-gray-600 mt-2">Modify notification setting: {{ $notificationSetting->name }}</p>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-lg overflow-hidden">
        <form method="POST" action="{{ route('admin.notification-settings.update', $notificationSetting) }}">
            @csrf
            @method('PUT')
            ...
            <div class="p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="type" id="type" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>
                            ...
                            <option value="email" {{ old('type', $notificationSetting->type) === 'email' ? 'selected' : '' }}>Email</option>
                            <option value="sms" {{ old('type', $notificationSetting->type) === 'sms' ? 'selected' : '' }}>SMS</option>
                            <option value="push" {{ old('type', $notificationSetting->type) === 'push' ? 'selected' : '' }}>Push Notification</option>
                            <option value="in_app" {{ old('type', $notificationSetting->type) === 'in_app' ? 'selected' : '' }}>In-App Notification</option>
                        </select>
                        @error('type')
                    ...
                    <div>
                        ...
                        <select name="category" id="category" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>
                            ...
                            <option value="booking" {{ old('category', $notificationSetting->category) === 'booking' ? 'selected' : '' }}>Booking</option>
                            <option value="user" {{ old('category', $notificationSetting->category) === 'user' ? 'selected' : '' }}>User</option>
                            <option value="company" {{ old('category', $notificationSetting->category) === 'company' ? 'selected' : '' }}>Company</option>
                            <option value="payment" {{ old('category', $notificationSetting->category) === 'payment' ? 'selected' : '' }}>Payment</option>
                            <option value="system" {{ old('category', $notificationSetting->category) === 'system' ? 'selected' : '' }}>System</option>
                        </select>
                        @error('category')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                ...
                <div>
                    ...
                    <input type="text" name="event" id="event" value="{{ old('event', $notificationSetting->event) }}" 
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $notificationSetting->name) }}" 
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500" 
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3" 
                              class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500" 
                              placeholder="Brief description of this notification">{{ old('description', $notificationSetting->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <!-- Email Template -->
                <div id="email-template" class="{{ $notificationSetting->type === 'email' ? '' : 'hidden' }}">
                    <div class="border-t pt-6">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="text" name="subject" id="subject" value="{{ old('subject', $notificationSetting->subject) }}" 
                                       class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500" 
                            ...
                            <div>
                                ...
                                <textarea name="body" id="body" rows="8" 
                                          class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500" 
                                          placeholder="Email body content">{{ old('body', $notificationSetting->body) }}</textarea>
                                @error('body')
                                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                ...
                <!-- SMS Template -->
                <div id="sms-template" class="{{ $notificationSetting->type === 'sms' ? '' : 'hidden' }}">
                    <div class="border-t pt-6">
                        ...
                        <div>
                            ...
                            <textarea name="sms_body" id="sms_body" rows="4" 
                                      class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500" 
                                      placeholder="SMS message content (max 160 characters)">{{ old('sms_body', $notificationSetting->sms_body) }}</textarea>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-4">
                            <div class="flex items-center">
                                <input type="checkbox" name="enabled" id="enabled" value="1" {{ old('enabled', $notificationSetting->enabled) ? 'checked' : '' }} 
                                       class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                                <label for="enabled" class="ml-2 block text-sm text-gray-900">Enabled</label>
                            </div>

                            <div class="flex items-center">
                                <input type="checkbox" name="auto_send" id="auto_send" value="1" {{ old('auto_send', $notificationSetting->auto_send) ? 'checked' : '' }} 
                                       class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="number" name="delay_minutes" id="delay_minutes" value="{{ old('delay_minutes', $notificationSetting->delay_minutes) }}" 
                                       class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500" 
                                       min="0" max="1440">
                                @error('delay_minutes')
                                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            ...
                            <div>
                                ...
                                <select name="priority" id="priority" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                                    <option value="low" {{ old('priority', $notificationSetting->priority) === 'low' ? 'selected' : '' }}>Low</option>
                                    <option value="normal" {{ old('priority', $notificationSetting->priority) === 'normal' ? 'selected' : '' }}>Normal</option>
                                    <option value="high" {{ old('priority', $notificationSetting->priority) === 'high' ? 'selected' : '' }}>High</option>
                                </select>
                                @error('priority')
                                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                @enderror
            ...
            <div class="px-6 py-4 bg-gray-50 border-t flex items-center justify-between">
                <div class="flex space-x-3">
                    ...
                    <form method="POST" action="{{ route('admin.notification-settings.destroy', $notificationSetting) }}" class="inline" 
